...
class DemographicsLoader:
    ...
    def load_tables(
        self, p_source: str, acs_source: str, nad_source: str
        ...
        logger.info(
            f"載入人口學資料完成：P={len(self.tables['P'])}人, "
    ...
    def _select_latest_visits(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        latest = (
            df.sort_values("visit_number", ascending=False).groupby("base_id").first()
        )
    ...
    def build_lookup_table(self) -> Dict[str, Dict[str, float]]:
        ...
        if "Sex" in merged.columns:
            ...
            if len(sex_values) > 0:
                mode_value = (
                    sex_values.mode()[0] if len(sex_values.mode()) > 0 else np.nan
                )
