...
class FeatureLoader:
        ...
        for subgroup in ["ACS", "NAD"]:
            if subgroup in self.allowed_subjects:
                ...
                if subgroup_path.exists():
                    subjects.extend(self._scan_filtered_subgroup(
                        subgroup_path,
                        group=subgroup,
        ...
        if "P" in self.allowed_subjects:
            ...
            if patient_path.exists():
                subjects.extend(self._scan_filtered_subgroup(
                    patient_path,
                    group="P",
                    allowed_ids=self.allowed_subjects["P"]
                ))
    ...
    def _scan_filtered_subgroup(
        ...
        for folder in path.iterdir():
            ...
            if not self._is_allowed(folder.name, base_id, str(subj_id_num), allowed_ids):
                continue
