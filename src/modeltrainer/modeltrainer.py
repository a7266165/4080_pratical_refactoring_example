...
# from src.modeltrainer.trainfactory.cross_validation import CrossValidator
...
#         self,
#         cv_methods: List[str],
#         classifiers: List  # 接受 ClassifierType enum 列表
#     ):
...
#             logger.info(f"  類別分布: 健康={sum(dataset['y']==0)}, 病患={sum(dataset['y']==1)}")
...
#                     # 從 enum 取得字串值
#                     classifier_name = classifier_enum.value if isinstance(classifier_enum, Enum) else str(classifier_enum)
...
#                             classifier_name
#                         )
#                         cv_results[classifier_name] = result
                        
#                         logger.info(f"      Acc={result['accuracy']:.3f}, MCC={result['mcc']:.3f}")
                        
#                     except Exception as e:
#                         logger.error(f"      失敗: {e}")
#                         cv_results[classifier_name] = {'error': str(e)}
...
#         # 設定特徵選擇
#         use_feature_selection = classifier_name in ["SVM", "Logistic Regression", "XGBoost"]
...
#             cv_method=cv_method,
#             n_folds=n_folds if n_folds else 5
#         )
...
#             feature_selection_method=feature_selection_method if use_feature_selection else "correlation"
#         )
        
#         # 處理結果格式
#         if isinstance(results, dict):
#             # 確保confusion_matrix是list格式
#             if 'confusion_matrix' in results and hasattr(results['confusion_matrix'], 'tolist'):
...
#         Args:
#             output_dir: 輸出目錄路徑
#             model_type: 模型類型 ("random_forest", "svm", "logistic", "xgboost")
#             random_state: 隨機種子
#         """
...
#             # 儲存模型與結果
#             self._save_model_and_results(dataset_key, results)
...
#         # 計算評估指標
#         train_metrics = self._calculate_metrics(y_train, y_pred_train, y_prob_train)
...
#         logger.info(f"交叉驗證準確率: {cv_scores.mean():.3f} (+/- {cv_scores.std():.3f})")
...
#     def _save_model_and_results(self, dataset_key: str, results: Dict[str, Any]):
#         """儲存模型與訓練結果"""
        
#         # 1. 儲存模型檔案
#         model_filename = f"{dataset_key}_{self.model_type}.pkl"
...
#             results_for_save['feature_importance'] = results_for_save['feature_importance'].tolist()
        
#         results_filename = f"{dataset_key}_{self.model_type}_results.json"
...
#         # 3. 儲存詳細報告
#         report_filename = f"{dataset_key}_{self.model_type}_report.txt"
...
#             f.write(f"模型類型: {self.model_type}\n")
...
#             f.write(f"\n交叉驗證:\n")
#             f.write(f"-" * 30 + "\n")
#             f.write(f"  平均準確率: {results['cv_mean']:.4f} (+/- {results['cv_std']:.4f})\n")
...
#         logger.info(f"開始訓練 {total_configs} 個配置 "
...
class ModelTrainer:
    ...
    def _calculate_metrics(
        ...
        metrics = {
            ...
            'recall': recall_score(y_true, y_pred, average='binary'),
            'f1': f1_score(y_true, y_pred, average='binary'),
            'mcc': matthews_corrcoef(y_true, y_pred),  # 加入 MCC
            'confusion_matrix': confusion_matrix(y_true, y_pred).tolist()
        }
