...
# # 模型訓練設定
# MODEL_TYPE = ["random_forest", "svm", "logistic", "xgboost"]
# TEST_SIZE = 0.2
# CV_FOLDS = 5
# RANDOM_SEED = 42
# OUTPUT_DIR = "./output"
...
#     if best_config:
#         logger.info("\n" + "="*50)
#         logger.info(f"最佳配置: {best_config}")
#         logger.info(f"測試準確率: {best_accuracy:.4f}")
#         logger.info("="*50)
...
#         logger.info(f"交叉驗證: {best_result['cv_mean']:.4f} (+/- {best_result['cv_std']:.4f})")
#         logger.info(f"F1 分數: {best_result['test_metrics']['f1']:.4f}")
#         if 'auc' in best_result['test_metrics']:
#             logger.info(f"AUC: {best_result['test_metrics']['auc']:.4f}")
#         logger.info("="*50)
